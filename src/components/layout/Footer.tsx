    ...
    <footer className="bg-dark border-t border-border mt-24">
      <div className="container mx-auto py-16 grid md:grid-cols-2 lg:grid-cols-5 gap-10">
        <div className="lg:col-span-1">
          ...
          <p className="text-sm opacity-70 mb-5 leading-relaxed">{siteConfig.brand.description}</p>
          <div className="flex gap-2">
            {socialLinks.map(({ Icon, href, label }) => (
              <a key={label} href={href} aria-label={label} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-lg border border-border bg-card/40 hover:border-primary hover:text-primary flex items-center justify-center transition-smooth">
        ...
        <div>
          ...
          <ul className="space-y-2.5 text-sm opacity-70">
            ...
            <li><Link to="/portfolio" className="hover:text-primary hover:opacity-100">Portfolio</Link></li>
            <li><Link to="/gallery" className="hover:text-primary hover:opacity-100">Gallery</Link></li>
        ...
        <div>
          ...
          <ul className="space-y-2.5 text-sm opacity-70">
            ...
            <li><Link to="/services/digital-marketing" className="hover:text-primary hover:opacity-100">Digital Marketing</Link></li>
            <li><Link to="/services/branding-design" className="hover:text-primary hover:opacity-100">Branding</Link></li>
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="space-y-2.5 text-sm opacity-70">
            ...
            <li><Link to="/testimonials" className="hover:text-primary hover:opacity-100">Testimonials</Link></li>
        ...
        <div>
          ...
          <ul className="space-y-3 text-sm opacity-70 mb-5">
            <li><a href={`mailto:${getEmail()}`} className="flex gap-2 hover:text-primary hover:opacity-100"><Mail className="w-4 h-4 mt-0.5 shrink-0" /> {getEmail()}</a></li>
            <li><a href={`tel:${getPhone('pk')}`} className="flex gap-2 hover:text-primary hover:opacity-100"><Phone className="w-4 h-4 mt-0.5 shrink-0" /> {getPhone('pk')}</a></li>
            <li className="flex gap-2">
  <MapPin className="w-4 h-4 mt-0.5 shrink-0 text-primary" /> 
  Pakistan (Islamabad, Rawalpindi, Rawat) & United Kingdom
</li>
          </ul>
          <Button asChild className="gradient-cta text-primary-foreground shadow-glow w-full">
            <Link to={siteConfig.cta.bookNow}><Calendar className="w-4 h-4 mr-2" /> Book Free Call</Link>
          </Button>
        </div>
      </div>
      <div className="border-t border-border">
        <div className="container mx-auto py-5 flex flex-col md:flex-row justify-between gap-3 text-sm opacity-60">
          <p>© {siteConfig.business.year} AM Enterprises. All Rights Reserved.</p>
