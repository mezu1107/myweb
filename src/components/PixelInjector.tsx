...
export const PixelInjector = () => {
  ...
  // Pinterest Pixel
  useEffect(() => {
    const id = data?.pinterest_pixel_id;
    if (!id || document.getElementById("pinterest-script")) return;
    const s = document.createElement("script");
    s.id = "pinterest-script"; s.async = true;
    s.innerHTML = `!function(e){if(!window.pintrk){window.pintrk=function(){window.pintrk.queue.push(Array.prototype.slice.call(arguments))};var n=window.pintrk;n.queue=[],n.version="3.0";var t=document.createElement("script");t.async=!0,t.src=e;var r=document.getElementsByTagName("script")[0];r.parentNode.insertBefore(t,r)}}("https://s.pinimg.com/ct/core.js");pintrk('load','${id}',{em:<user_id>});pintrk('page');`;
    document.head.appendChild(s);
  }, [data?.pinterest_pixel_id]);

  // Snapchat Pixel
  useEffect(() => {
    const id = data?.snapchat_pixel_id;
    if (!id || document.getElementById("snapchat-script")) return;
    const s = document.createElement("script");
    s.id = "snapchat-script"; s.async = true;
  ...
  // Twitter/X Pixel
  useEffect(() => {
    const id = data?.twitter_pixel_id;
    if (!id || document.getElementById("twitter-script")) return;
    const s = document.createElement("script");
    s.id = "twitter-script"; s.async = true;
    s.src = "https://platform.twitter.com/oct.js";
    document.head.appendChild(s);
    const inline = document.createElement("script");
    inline.innerHTML = `twttr.conversion.trackPid('${id}', { tw_sale_amount: 0, tw_order_quantity: 0 }, false);`;
    document.head.appendChild(inline);
  }, [data?.twitter_pixel_id]);

  // Hotjar
  useEffect(() => {
    const id = data?.hotjar_site_id;
    if (!id || document.getElementById("hotjar-script")) return;
    const s = document.createElement("script");
    s.id = "hotjar-script";
    s.innerHTML = `(function(h,o,t,j,a,r){h.hj=h.hj||function(){(h.hj.q=h.hj.q||[]).push(arguments)};h._hjSettings={hjid:${id},hjsv:6};a=o.getElementsByTagName('head')[0];r=o.createElement('script');r.async=1;r.src=t+h._hjSettings.hjid+j+h._hjSettings.hjsv;a.appendChild(r);})(window,document,'https://static.hotjar.com/c/hotjar-','.js?sv=');`;
    document.head.appendChild(s);
  }, [data?.hotjar_site_id]);

  // Microsoft Clarity
  useEffect(() => {
    const id = data?.clarity_project_id;
